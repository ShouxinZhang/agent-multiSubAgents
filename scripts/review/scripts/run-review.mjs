...
const buildStep = runCommand('build-check', 'bash', ['scripts/check_errors.sh'], {
  cwd: projectRoot,
  logPath: buildLog
});

const testStep = runCommand('unit-test', 'npm', ['run', '-w', 'web', 'test'], {
  cwd: projectRoot,
  logPath: testLog
});
...
const dependencyStep = runCommand('dependency-gate', 'node', [
  ...
  projectRoot,
  '--policy',
  path.join(reviewRoot, 'config/policy.json'),
  '--output',
  dependencyFile
], {
  cwd: projectRoot,
...
if (testStep.code !== 0) {
  finalStatus = 'block';
  blockReasons.push('自动化测试失败（npm run -w web test）');
}
